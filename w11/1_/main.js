...
let innerLimit = degToRad(20);
let outerLimit = degToRad(40);

let uLightDirectionLoc = gl.getUniformLocation(program,"u_lightDirection");
let uInnerLimitLoc = gl.getUniformLocation(program,"u_innerLimit");
let uOuterLimitLoc = gl.getUniformLocation(program,"u_outerLimit");
...
let i_innerLimit = document.querySelector("#i-inner-limit");
let i_outerLimit = document.querySelector("#i-outer-limit");
...
i_innerLimit.addEventListener("input",e=>{
    innerLimit = degToRad(i_innerLimit.valueAsNumber);
});
i_outerLimit.addEventListener("input",e=>{
    outerLimit = degToRad(i_outerLimit.valueAsNumber);
});
...
function drawScene(){
    ...
    gl.uniform1f(uInnerLimitLoc,Math.cos(innerLimit));
    gl.uniform1f(uOuterLimitLoc,Math.cos(outerLimit));
